...
import { getFinanceTasks, updateFinanceTaskStatus } from '../../../api/financeApi';
...
const FinanceTasks = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getFinanceTasks();
      setTasks(res.data || []);
    } catch (err) {
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    try {
      await updateFinanceTaskStatus(taskId, { status: newStatus });
  ...
  return (
    <div>
      <PageHeader title="My Finance Tasks" subtitle="Tasks assigned to you by the system or other managers">
      ...
      <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.04)', border: '1px solid #f1f5f9' }}>
        ...
        {loading ? (
          ...
          <EmptyState icon={FaTasks} title="No tasks found" message="You don't have any finance tasks matching this filter." />
        ) : (
